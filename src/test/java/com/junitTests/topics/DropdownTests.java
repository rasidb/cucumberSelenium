...
public class DropdownTests {
    ...
    void dropdownTest() {
        ...
        assertEquals("1999", select.getFirstSelectedOption().getText());

        // Handle the month dropdown
        WebElement month = Driver.getDriver().findElement(By.id("month"));
        select = new Select(month);
        select.selectByVisibleText("May");
        assertEquals("May", select.getFirstSelectedOption().getText());
        ...
        assertEquals("Arkansas", select.getFirstSelectedOption().getText());
    }
